...
(define (interpret stmt state tmap)
  ;(display stmt) (newline) (display state) (newline)
  (let ((kind (car stmt)))
    (cond ((equal? kind 'block) (interpret-block stmt state tmap))
          ((equal? kind 'declare) (list 'flag (interpret-declaration stmt state) (update-typemap stmt tmap)))
          ((equal? kind ':=) (interpret-assignment stmt state tmap))
          ((equal? kind 'if) (interpret-if stmt state tmap))
          ((equal? kind 'sprint) (interpret-sprint stmt state tmap))
          ((equal? kind 'while) (interpret-while stmt state tmap))
...
(define (loophole stmt state tmap)
  (let ([thing1 (interpret (cadr stmt) state tmap)]
        [thing2 (cons 'block (cddr stmt))])
    (cond ((equal? 'flag (car thing1)) (interpret thing2 (cadr thing1) (caddr thing1)))
          (else (interpret thing2 thing1 tmap)))))

;;INTERPRET BLOCK ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(define (interpret-block stmt state tmap)
  (cond ((only-blocks? stmt) state)
        (else (loophole stmt state tmap))))

;;INTERPRET DECLARATION
(define (interpret-declaration stmt state)
  (state-add state (declaration-get-var stmt)))

(define (update-typemap stmt tmap)
  (typemap-add tmap (cdr stmt)))

;;INTERPRET ASSIGNMENT
(define (interpret-assignment stmt state tmap)
  (state-update state (assignment-get-var stmt) (eval-exp (assignment-get-exp stmt) state tmap)))

;;INTERPRET IF
(define (interpret-if stmt state tmap)
  (let ([test (eval-exp (if-get-test stmt) state tmap)]
        [then (if-get-then stmt)])
    (cond ((and (not test) (if-has-else? stmt)) (interpret (if-get-else stmt) state tmap))
          (test (interpret then state tmap))
          (else state tmap))))

;;INTERPRET WHILE
(define (interpret-while stmt state tmap)
  (let ([test (eval-exp (while-get-test stmt) state tmap)]
        [body (while-get-body stmt)])
    (cond (test (interpret-while stmt (interpret body state tmap) tmap))
          (else state tmap))))

(define (printstringexp stmt state tmap)
  (display (sprint-get-label stmt)) (display (eval-exp (sprint-get-exp stmt) state tmap)) (newline)
  state)
...
;;INTERPRET SPRINT
(define (interpret-sprint stmt state tmap)
  (display "STATEMENT") (display stmt) (newline)
  (cond ((sprint-has-exp? stmt) (printstringexp stmt state tmap))
        (else (printstring stmt state))))

(define (other symbol)
  (if (equal? 'int symbol) "boolean" "int"))
 
(define (printerror exp type tmap)
  (display (string-append " TYPE IN (" (exp->string exp)
                          ") TYPE SHOULD BE (" (symbol->string type)
                          ") BUT THE TYPE IS (" (other type)")"))
  #f)
...
;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(define (eval-exp exp state tmap)
  (cond ((int? exp) exp)
        ((bool? exp) (symbol->bool exp))
        ((and (map-contains state exp)
              (not (equal? 'undefined (state-get-value state exp)))) (state-get-value state exp))
        ((unary? exp) (eval-unary exp state tmap))
        (else (eval-binary exp state tmap))))

;;EVALUATE BINARY
(define (eval-binary exp state tmap)
  (cond ((not (valid-exp? exp tmap)) state)
        ((equal? '+ (exp-get-operator exp)) (+ 
                                              (eval-exp (exp-get-left-operand exp) state tmap)
                                              (eval-exp (exp-get-right-operand exp) state tmap)))
        ((equal? '- (exp-get-operator exp)) (-
                                              (eval-exp (exp-get-left-operand exp) state tmap)
                                              (eval-exp (exp-get-right-operand exp) state tmap)))
        ((equal? '* (exp-get-operator exp)) (*
...
;;EVALUATE UNARY
(define (eval-unary exp state tmap) 
  (cond ((equal? '~ (exp-get-operator exp)) (not (eval-exp (cadr exp) state tmap)))
        ((equal? '- (exp-get-operator exp)) (* -1 (eval-exp (cadr exp) state tmap)))
        (else 'syntaxman)))
...
(define (type-match? type exp tmap)
  (if (not (equal? type (type-of? exp tmap))) (printerror exp type tmap) #t))
