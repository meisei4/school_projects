...
(define (innerjoin-onkey inner outer)
  (cond ((null? inner) )
        ((map-contains outer (car (car inner))) (cons (car inner) (innerjoin-onkey (cdr inner) outer)))
        (else (innerjoin-onkey (cdr inner) outer))))

;;INTERPRET BLOCK ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;UNKNOWN
(define (interpret-block stmt state)
  (let* ((outerblock state)
        (innerblock (interpret (cadr stmt) state)))
     (cond ((null? stmt) state)
           (else (interpret (cons 'block (cddr stmt)) (innerjoin-onkey innerblock outerblock))))))
...
(define (eval-exp exp state)
  (cond ((int? exp) exp)
        ...
        ((and (unary? exp) (eval-unary exp state)))
        ((binary? (eval-binary exp state)))
        (else 'undefined)))
...
(define (valid-if? stmt tmap)
  (let ([test (if-get-test stmt)]
        [then (if-get-then stmt)])
    (cond ((if-has-else? stmt) (and (type-match? 'boolean test tmap)
