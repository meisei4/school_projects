...
(define (interpret stmt state tmap)
  ...
  (let ((kind (car stmt)))
    (cond ((equal? kind 'block) (interpret-block stmt state tmap))
          ((equal? kind 'declare) (interpret-declaration stmt state tmap))
...
;;INTERPRET BLOCK ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;UNKNOWN
(define (interpret-block stmt state tmap)
  (cond ((only-blocks? stmt) state tmap)
        (else (interpret (cons 'block (cddr stmt)) (interpret (cadr stmt) state tmap) tmap))))

;;INTERPRET DECLARATION
(define (interpret-declaration stmt state tmap)
...
(define (interpret-while stmt state tmap)
  (let ([test (eval-exp (while-get-test stmt) state tmap)]
    ...
    (cond (test (interpret-while stmt (interpret body state tmap)))
          (else state tmap))))
...
(define (printerror exp tmap)
  (display "type error in " (exp->string exp)))
...
(define (eval-binary exp state tmap)
  (cond ((valid-binary? exp tmap) (error "type error with" (exp->string exp)))
...
(define (valid-assign? assign tmap)
  (let ([var (assignment-get-var assign)]
    ...
    (cond ((not (map-contains tmap var)) (printerror var tmap))
          ((not (valid-exp? exp tmap)) (printerror exp tmap))
          ((not (type-match? type exp tmap)) (printerror assign tmap))
          (else #t))))
...
(define (valid-while? stmt tmap)
  (let ([test (while-get-test stmt)]
        [body (while-get-body stmt)])
    (cond ((not (type-match? 'boolean test tmap)) (printerror test tmap)))
          (else (valid-stmt? body tmap))))
...
(define (valid-if? stmt tmap)
  (let ([test (if-get-test stmt)]
    ...
    (cond ((if-has-else? stmt) (and (valid-stmt? then tmap)
          ...
          (else (cond ((not (type-match? 'boolean test tmap)) (printerror test tmap))
                      (else (valid-stmt? then tmap)))))))
...
(define (valid-block? blk tmap)
  (let ((update (valid-declarations? (block-get-declarations blk) tmap)))
    (valid-body? (block-get-body blk) update)))
