...
sendWork(void * arg) {
  ...
  while (1) {
    ...
    if (p == NULL) break;
    int cnt = strlen(buf);
    write(data->sockfd,buf,cnt);
  }
  fprintf(stderr,"sendWork exiting\n");
  pthread_cancel(data->recvThread);
  pthread_exit(0);
  return NULL;
}
...
recvWork(void * arg) {
  ...
  while (1) {
    int cnt = read(data->sockfd,buf,bufsize);
    if (cnt <= 0) break;
    fwrite(buf,1,cnt,stdout);    
  }
  fprintf(stderr,"recvWork exiting\n");
  pthread_cancel(data->sendThread);
  pthread_exit(0);
  return NULL;
}
...
actAsServer(int portNum) {
  ...
  ret = pthread_create(&(data.sendThread),NULL,sendWork,(void *)&data);
  exitIfError(ret,"pthread_create failed\n");
  
  pthread_t recvThrd;
  ret = pthread_create(&(data.recvThread),NULL,recvWork,(void *)&data);
  exitIfError(ret,"pthread_create failed\n");
  
  pthread_join(data.sendThread,NULL);
  pthread_join(data.recvThread,NULL);
